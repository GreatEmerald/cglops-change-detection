...
APUPlot = function(dflist, changeonly=FALSE, nbins=33)
{
    ...
    # Plot results
    ggplot(BinAS, aes(x=bin, y=Accuracy)) + geom_line(aes(colour="Accuracy")) + geom_line(aes(y=F1Score, colour="F1Score")) + geom_line(aes(y=Sensitivity, colour="Sensitivity")) + geom_line(aes(y=Precision, colour="Precision")) + geom_line(aes(y=Specificity, colour="Specificity")) +
        ...
        scale_x_continuous(name="Probability of no break", breaks=c(0, 0.001, 0.01, 0.05, 0.25, 1), trans = pow4_trans, limits = c(0,1)) + ylab("Statistic") +
...
```
```{r runapuplot4}
APUPlot(list(BFMonitor=bfmres1y, BFLiteBIC=bf0nres1y, BFLiteLWZ=bf0nres1ylwz, BFASTstlplus=bfres1y))
```

```{r runapuplot4change}
APUPlot(list(BFMonitor=bfmres1y, BFLiteBIC=bf0nres1y, BFLiteLWZ=bf0nres1ylwz, BFASTstlplus=bfres1y), changeonly=TRUE, nbins=11) # ~300 points per bin
```
