...
# Slightly longer than the actual number of weeks in a year, so we don't have two breaks in a year
GetBreakNumber = function(dates)
{
    1/(as.numeric(difftime(max(dates), min(dates), units="weeks"))/52.5)
}
...
SparkCalc = function(input_raster, fx, filename, mem_usage=0.9*1024^3, datatype=NULL, options=NULL)
{
    ...
    ## Workaround for a bug in SparkR that causes "No status is returned" after 100 minutes. Set to 2 weeks.
    connectBackend.orig = getFromNamespace('connectBackend', pos='package:SparkR')
    connectBackend.patched = function(hostname, port, timeout = 3600*24*7*2) {
...
GetLastBreakInTile = function(pixel)
{
    ...
    #    cat(c("ERROR: Duplicate breakpoint years! Years:", BreakpointYears, "Dates:", sapply(ConfInts[,"breakpoints"], BreakpointDate, bpp), "Breakpoints:", ConfInts[,"breakpoints"], "h:", GetBreakNumber(dates), "\n"))
    #BreakpointDays = sapply(ConfInts, BreakpointToDateSinceT0, bpp, t0) # Convert indices to days since t0
    #cat(c("BreakpointDays:", BreakpointDays, "\n"))
...
SparkCalc(timeseries, GetLastBreakInTile, "../../tmp/noconfint-X16Y06-order3.tif", datatype="INT2S", options="COMPRESS=DEFLATE")
