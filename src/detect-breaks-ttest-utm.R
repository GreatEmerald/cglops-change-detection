    ...
    scalc = function(FileIndex)
    {
        ...
        if (TRUE)
        {
            ...
            , error=function(e){
                ...
                ResultChunk = gdal_translate(GrdFilename, tmpfilename, ot="Int16", verbose=TRUE, output_Raster=TRUE)
                print("List of files in ./tmp:")
                print(list.files("tmp"))
            })
...
SparkCalc(InputFiles, TTestBreaks, args[["output-dir"]], datatype="FLT4S", mem_usage=1024^3)#, options="COMPRESS=DEFLATE")
